...
struct SATitleDetailsView: View {
    ...
    var body: some View {
        if let program = programDetails, let omdbInfo = programDetails?.omdbResult {
            //Text("Program Title: \(program.title) Omdb Title: \(omdbInfo.imdbID)")
            NavigationView {
...
struct SATitleDetailsView_Previews: PreviewProvider {
    static let programDetails = ProgramDetails(firstAirYear: 2024, originalTitle: "Billy Loves Jenny", creators: ["BF","JB"], imdbId: "tt2802850", tmdbId: 222, type: "RomCom", title: "Billy and Jenny Get Down", lastAirYear: 2024, genres: [Genre](), seasonCount: 1, episodeCount: 1, status: Status(statusText: "Done", statusCode: 1), streamingInfo: nil)
